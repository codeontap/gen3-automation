...
function main() {
  ...
  ### Tags ###
      ...
      if [[ "${SEGMENT}" == "default" ]]; then
        ...
        define_context_property "RELEASE_TAG" "${AUTOMATION_RELEASE_IDENTIFIER}-${SEGMENT}-${DEPLOYMENT_LOCATION}"
        define_context_property "DEPLOYMENT_TAG" "${AUTOMATION_DEPLOYMENT_IDENTIFIER}-${SEGMENT}-${DEPLOYMENT_LOCATION}"
      fi
